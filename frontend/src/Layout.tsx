...
function Layout() {
  ...
  const { showAlert, showUploadFileResult, updateShowUploadFileResult } =
    useStoreContext();
  ...
  function calculate(result: ICurrentValue[]) {
    let initialValue = Object.keys(CLINICAL_SCALES).reduce<any>(
      (total, currentValue, currentIndex, baseValue) => {
        total.push({
    ...
    let newResult = result.reduce<any>(
      (total, currentValue, currentIndex, baseValue) => {
        const { index, answer, questions } = currentValue;

        Object.entries(CLINICAL_SCALES).forEach(
          ([categoryName, { _true, _false }]: any) => {
            const categoryIndex = total.indexOf(
              total.find((item: any) => item.category === categoryName)
            );
  ...
  async function showResult() {
    ...
    if (!password) return;
    if (password !== process.env.REACT_APP_PASSPHASE)
    ...
    if (password && cypherText) {
      try {
        ...
        let [finalResult, answeredCollection] = calculate(
        ...
        const worksheetAnsweredCollection: any = await XLSX.utils.json_to_sheet(
          answeredCollection
        );
        const workbook = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(
          workbook,
          worksheetFinalResult,
          "BPI Result"
        );
        XLSX.utils.book_append_sheet(
          workbook,
          worksheetAnsweredCollection,
          "BPI Answered"
        );
    ...
    <>
      <Modal show={showUploadFileResult}>
        ...
        <div className="grid grid-cols-2">
          <button
            type="button"
            className="text-slate-100 bg-slate-900 p-2 px-4 rounded-lg justify-self-start"
            onClick={showResult}
          >
            Download result
          </button>
          <button
            type="button"
            className="justify-self-end text-slate-900 border border-lg border-slate-900 bg-slate-100 p-2 px-4 rounded-lg"
            onClick={updateShowUploadFileResult}
          >
            Close
          </button>
        </div>
      ...
      <div className="bg-sky-200 max-w h-screen">
